...
`page.dc.html` (the entry page) renders that record twice, from the
...
`ZoomedPlatformIcon.dc.html`, `page.dc.html`, `images/platforms/`, and
...
`components.js` (next to `page.dc.html` and the sibling `*.dc.html`
files) is a **derived build artifact** — it inlines every sibling
`*.dc.html` component (all except the entry page, `page.dc.html`). The
